/**
 * 0G Storage — uploads via official SDK + user's MetaMask wallet.
 * HTTP storage node calls are relayed through /api/storage-relay (no server wallet needed).
 */
...
function friendlyUploadError(message: string): string {
  if (message.includes('User rejected') || message.includes('user rejected')) {
    return 'Storage upload rejected in wallet — approve the storage fee transaction';
  }
  if (message.includes('insufficient funds')) {
    return 'Insufficient 0G in wallet for storage fees — you need testnet 0G on-chain (not just router deposit)';
  }
  if (message.includes('Network Error') || message.includes('Mixed Content')) {
    return 'Storage relay unreachable — redeploy with /api/storage-relay enabled';
  }
  return message;
}
...
/**
 * Upload evaluation data to 0G Storage using the connected wallet for storage fees.
 */
export async function uploadToStorage(data: StorageData): Promise<StorageResult> {
  ...
  try {
    ...
    if (uploadErr !== null) {
      ...
      return { success: false, rootHash: null, error: friendly };
    }
